...
class TestChirpsOptions:
    ...
    def test_chirps_options_contains_expected_datasets(self):
        ...
        assert all(
            dataset in result["dataset"].values
...
class TestCheckChirpsAvailable:
    ...
    @pytest.mark.skipif(
        skip_if_chirps_down(), reason="CHIRPS server not accessible"
    )
    ...
    def test_check_chirps_available_filters_tif_gz_files_correctly(self):
        ...
        if result is not None and len(result) > 0:
            ...
            assert all(
                fname.endswith(".tif.gz") for fname in result["file_name"]
            )
...
class TestDownloadChirps:
    ...
    def test_download_chirps_skips_existing_files(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            existing_file = (
                temp_path / "africa_monthly_chirps-v2.0.2020.01.tif"
            )
    ...
    def test_workflow_chirps_options_to_download(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("requests.get") as mock_get:
                ...
                try:
                    download_chirps(
                        dataset=dataset, start="2020-01", out_dir=temp_dir
                    )
                except Exception:
